...
Deploy, auto-sync, detect drift, and manually synchronize Kubernetes applications using Argo CD on Red Hat OpenShift.
...
- How to deploy an application using Argo CD on Red Hat OpenShift
...
## Installing Argo CD on Red Hat OpenShift

On Red Hat OpenShift, install Argo CD using the Red Hat OpenShift GitOps Operator.
...
Installing Argo CD through the Red Hat OpenShift GitOps Operator ensures that the cluster automatically manages Argo CD’s lifecycle. As a result, upgrades, RBAC integration, and component health are consistently managed using Kubernetes-native patterns.
