#!/bin/bash
##
# Script to mirror IBM Backup and Restore images before install, if going to enable IBM Backup and Restore service.
...
For validation of images presence in enterprise registry, execute as following:
 ./mirror-bkp-restore-images.sh -rh "ENTERPRISE_REGISTRY_HOST" -il "IMAGE_LIST_JSON" -tp "TARGET_PATH" -p "ENTERPRISE_REGISTRY_PORT" -ps "absolute path for config.json directory"
Available options:
-il : Mandatory absolute path to image list json to be validated in registry
-ps : Fully qualified directory path for config.json file. If not provided ~/.docker/config.json file will be used for authentication.
...
Example:
./mirror-bkp-restore-images.sh -ps "/root/mirror" -rh "test-jfrog-artifactory.com" -tp "hci-260" -p "443" -il "isf-260-images.json"
EOF
    exit 1
}
...
function read_image_list {
  ...
  BKP_RH_OPENSHIFT4=$((cat ${IMAGE_LIST_JSON}  | jq -r '.images."backup-restore"."redhat-openshift4" | @sh') | tr -d '"' | tr -d \')
  BKP_RH_INDEX=$((cat ${IMAGE_LIST_JSON}  | jq -r '.images."backup-restore"."redhat-index" | @sh') | tr -d '"' | tr -d \')
}

# Function to mirror guardian images
function mirror_guardian_images {
  	for img in ${BKP_BITNAMI[@]}; do
      		print debug "skopeo copy --authfile $PULLSECRET --all docker://${DOCKER}/${BITNAMI}/${img} docker://${LOCAL_REGISTRY}/${MIRROR_PATH}/${img}  2>&1 > ${MIRROR_LOG}"
      		skopeo copy --authfile $PULLSECRET --all docker://${DOCKER}/${BITNAMI}/${img} docker://${LOCAL_REGISTRY}/${MIRROR_PATH}/${img}  2>&1 >> ${MIRROR_LOG}
      		if [[ $? -ne 0 ]] ; then print error "Failed to copy ${img} from ${DOCKER}/${BITNAMI} to ${LOCAL_REGISTRY}/${MIRROR_PATH}"; failedtocopy=1; fi
  	done
	...
	for img in ${BKP_FBR[@]}; do
		print debug "skopeo copy --authfile $PULLSECRET --all docker://${IBM_REGISTRY}/${IBMPATH}/${FBR}/${img} docker://${LOCAL_REGISTRY}/${MIRROR_PATH}/${img}  2>&1 > ${MIRROR_LOG}"
		skopeo copy --authfile $PULLSECRET --all docker://${IBM_REGISTRY}/${IBMPATH}/${FBR}/${img} docker://${LOCAL_REGISTRY}/${MIRROR_PATH}/${img}  2>&1 >> ${MIRROR_LOG}
		if [[ $? -ne 0 ]] ; then print error "Failed to copy ${img} from ${IBM_REGISTRY}/${IBMPATH}/${FBR} to ${LOCAL_REGISTRY}/${MIRROR_PATH}"; failedtocopy=1; fi
	done

	if [[ $failedtocopy -eq 1  ]] ; then
    print error "Some Backup and restore operator images are having issues to copy, please check nohup.out / output of execution"
  else
    print info "mirror for Backup and Restore images done successfully"
  fi
}
...
#$CTOOL login ${QUAY} -u ${QUAY_USER} -p ${QUAY_PASSWORD}
#if [[ $? -ne 0 ]] ; then print error "Login to ${QUAY} failed. Ensure you have correct credentials in ${PULLSECRET} and check connectivity to registry."; exit 1; fi

mirror_guardian_images
