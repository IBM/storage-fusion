# Backup & Restore Hotfixes for Version 2.13.1

This document lists all hotfixes available for IBM Storage Fusion Backup & Restore version 2.13.1
...
- [`br-post-install-patch-2.13.1.sh`](br-post-install-patch-2.13.1.sh) 
- [`br-2.13.1patch-offline-mirror.sh`](br-2.13.1patch-offline-mirror.sh) 
...
```bash
./br-2.13.1patch-offline-mirror.sh <TARGET_PATH>
```
...
**Example:**
```bash
./br-2.13.1patch-offline-mirror.sh myregistry.example.com:5000/isf-images
```
...
1. **Download the hotfix scripts** to your local system:
   ...
   ls -lh br-post-install-patch-2.13.1.sh
   ls -lh br-2.13.1patch-offline-mirror.sh
   ```

2. **Make the script executable:**
   ```bash
   chmod +x br-post-install-patch-2.13.1.sh
   ```
...
3. **Execute the hotfix script** with the appropriate deployment type flag:
   ...
   **For HCI deployments:**
   ```bash
   ./br-post-install-patch-2.13.1.sh -hci
   ```

   **For SDS deployments:**
   ```bash
   ./br-post-install-patch-2.13.1.sh -sds
   ```


## Prerequisites

- IBM Storage Fusion Backup & Restore version 2.13.1 must be installed
...
- `br-post-install-patch-2.13.1.sh` - Main hotfix installation script
- `br-2.13.1patch-offline-mirror.sh` - Offline image mirroring script
