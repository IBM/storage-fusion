# Backup & Restore Hotfixes for Version 2.12.3

This document lists all hotfixes available for IBM Storage Fusion Backup & Restore version 2.12.3
...
## Hotfix hotfix-2.12.3.1 (Latest)

**Release Date:** Sep 23, 2026

### Issues Fixed

1. Backups on Fusion 2.11+ are broken with OADP-1.4.11 with custom CA certificate due to breaking OADP change

### Required Files

Before applying the hotfix, download the following files:

- [`br-post-install-patch-2.12.3.sh`](br-post-install-patch-2.12.3.sh) 
- [`br-2.12.3patch-offline-mirror.sh`](br-2.12.3patch-offline-mirror.sh) 
...
```bash
./br-2.12.3patch-offline-mirror.sh <TARGET_PATH>
```
...
**Example:**
```bash
./br-2.12.3patch-offline-mirror.sh myregistry.example.com:5000/isf-images
```
...
1. **Download the hotfix scripts** to your local system:
   ...
   ls -lh br-post-install-patch-2.12.3.sh
   ls -lh br-2.12.3patch-offline-mirror.sh
   ```

2. **Make the script executable:**
   ```bash
   chmod +x br-post-install-patch-2.12.3.sh
   ```
...
3. **Execute the hotfix script** with the appropriate deployment type flag:
   ...
   **For HCI deployments:**
   ```bash
   ./br-post-install-patch-2.12.3.sh -hci
   ```

   **For SDS deployments:**
   ```bash
   ./br-post-install-patch-2.12.3.sh -sds
   ```


## Prerequisites

- IBM Storage Fusion Backup & Restore version 2.12.3 must be installed
...
- `br-post-install-patch-2.12.3.sh` - Main hotfix installation script
- `br-2.12.3patch-offline-mirror.sh` - Offline image mirroring script
