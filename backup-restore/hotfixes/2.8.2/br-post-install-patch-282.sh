   ...
   if (oc get deployment -n $BR_NS job-manager -o yaml > $DIR/job-manager-deployment.save.yaml)
    ...
    if (oc get deployment -n $BR_NS backup-service -o yaml > $DIR/backup-service-deployment.save.yaml)
      then
        ...
        oc patch deployment backup-service -n $BR_NS -p '{"spec":{"template":{"spec":{"containers":[{"name":"backup-service","image":"cp.icr.io/cp/fbr/guardian-backup-service@sha256:e26efc87da06657c54aedc7f048855adf8c1fc76c6f3bd35dfa2e545caadbe98"}]}}}}'
    else
...
if (oc get deployment -n $BR_NS transaction-manager -o yaml > $DIR/transaction-manager-deployment.save.yaml)
  then
    ...
    oc set image deployment/transaction-manager -n $BR_NS transaction-manager=cp.icr.io/cp/fbr/guardian-transaction-manager@sha256:c2b1a95f339ab7452a6a1a95def75bed77493dbf0982a9fb7858f81b22d4cce2
else
