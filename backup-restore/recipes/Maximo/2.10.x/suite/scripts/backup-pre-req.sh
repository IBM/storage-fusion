...
# Apply RBAC for Suite, SLS, MongoDB, CSVs.
echo -e "\n=== Creating Roles and RoleBindings for Suite, SLS, MongoDB, CSV ==="

oc apply -f - <<EOF
apiVersion: rbac.authorization.k8s.io/v1
kind: Role
metadata:
  name: allow-suites-access
...
kind: Role
metadata:
  name: allow-licenseservices-access
...
kind: RoleBinding
metadata:
  name: bind-licenseservices-access
  namespace: ${SLS_NAMESPACE}
subjects:
  - kind: ServiceAccount
    name: transaction-manager
...
kind: Role
metadata:
  name: allow-mongodbcommunity-access
...
kind: RoleBinding
metadata:
  name: bind-mongodbcommunity-access
  namespace: ${MONGODB_NAMESPACE}
subjects:
  - kind: ServiceAccount
    name: transaction-manager
