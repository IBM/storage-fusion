...
echo -e "\n=== Secrets ==="
oc get -n $MANAGE_NAMESPACE secret -l for-backup=true --show-labels
...
recipe_name="manage/maximo-child-manage-backup-restore.yaml"
local_recipe="manage/maximo-child-manage-backup-restore-local.yaml"
echo -e "\n=== Creating recipe from template ==="
if [[ -f $recipe_name ]]; then
  awk -v mas_id="$MAS_INSTANCE_ID" \
    -v manage_ns="$MANAGE_NAMESPACE" \
    '{gsub(/\$\{MAS_INSTANCE_ID\}/, mas_id); \
      gsub(/\$\{MANAGE_NAMESPACE\}/, manage_ns); \
...
# Add manage namespace to includedNamespaces in core child recipe
local_core_recipe="core/maximo-child-core-backup-restore-local.yaml"
INCLUDE_NAMESPACE="${MANAGE_NAMESPACE}" yq -i '(.spec.groups[] | select(.name == "maximo-core-resources") | .includedNamespaces) |= . + [env(INCLUDE_NAMESPACE)]' ${local_core_recipe}
...
add_rule_if_missing ${CLUSTERROLE} "apps.mas.ibm.com" "manageapps"
add_rule_if_missing ${CLUSTERROLE} "apps.mas.ibm.com" "manageworkspaces"

# check if namespace is added to Fusion App CR.
add_ns_if_missing_from_fapp ${MANAGE_NAMESPACE}
