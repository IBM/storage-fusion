...
class TestEndToEndWorkflows:
    ...
    def test_authenticate_list_select_query_workflow(
        self: Any,
        mock_get: Mock,
        mock_post: Mock,
        mock_run: Mock,
        ...
        mock_get_response.json.return_value = {
            "data": [{"name": "vector-store-1"}, {"name": "vector-store-2"}]
        }
        ...
        mock_post_response.json.return_value = {
            "success": True,
            "data": [{"text": "Result 1"}, {"text": "Result 2"}],
        }
    ...
    def test_vector_search_retrieve_file_query_workflow(
        self: Any,
        mock_run: Mock,
        sample_config: dict[str, Any],
        mock_logger: Any,
        ...
        vector_store_service = VectorStoreService(
            sample_config, mock_auth_service, mock_logger
        )
        ...
        vector_stores: list[str] = vector_store_service.list_vector_stores(use_cache=False)
...
class TestServiceIntegration:
    ...
    @pytest.mark.integration
    def test_auth_service_token_used_by_query_service(
        self: Any,
        sample_config: dict[str, Any],
        mock_logger: Any,
        mock_cache_service: Any,
        ...
        query_service = QueryService(
            sample_config, mock_logger, auth_service=auth_service
        )
    ...
    @pytest.mark.integration
    def test_cache_service_used_across_services(
        self: Any,
        sample_config: dict[str, Any],
        mock_logger: Any,
        mock_cache_service: Any,
        ...
        result: list[str] = query_service.list_vector_stores(use_cache=True)
    ...
    @pytest.mark.integration
    def test_metrics_service_tracks_operations(
        self: Any,
        sample_config: dict[str, Any],
        mock_logger: Any,
        mock_metrics_service: Any,
        ...
        with patch(
            "chatbot.services.llm_service.LLMService._call_nvidia"
        ) as mock_nvidia:
...
class TestTokenRefreshIntegration:
    ...
    @pytest.mark.slow
    @patch("chatbot.services.auth_service.subprocess.run")
    def test_token_refresh_during_long_session(
        self,
        mock_run: Any,
        sample_config: dict[str, Any],
        mock_logger: Any,
    ...
    @pytest.mark.integration
    @patch("chatbot.services.auth_service.subprocess.run")
    def test_operations_continue_after_token_refresh(
        self,
        mock_run: Any,
        sample_config: dict[str, Any],
        mock_logger: Any,
        ...
        with patch("chatbot.services.query_service.requests.post") as mock_post:
            ...
            call_args: _Call | Any = mock_post.call_args
