...
def sample_config() -> dict[str, Any]:
    ...
    return {
        "console_url": "https://console-openshift-console.apps.test-cluster.com",
        "oc_username": "test-user",
        "oc_password": "test-password",
        "cas_url": "https://console-ibm-spectrum-fusion-ns.apps.test-cluster.com/cas/api/v1",
        "cas_namespace": "ibm-cas",
        "default_vector_store": "test-vector-store",
        "default_limit": 5,
        ...
        "nvidia_llm_url": "http://test-nvidia-endpoint",
...
def mock_metrics_service() -> Mock:
    ...
    metrics.get_statistics = Mock(
        return_value={"total_requests": 100, "total_errors": 5, "uptime_seconds": 3600}
    )
    return metrics


@pytest.fixture
def mock_auth_service(sample_config: dict[str, Any], mock_logger: Any, mock_cache_service: Any) -> Mock:
...
@pytest.fixture
def sample_vector_stores() -> list[str]:
...
def sample_query_result() -> dict[
    str, bool | list[dict[str, dict[Any, Any] | str]] | str
]:
...
@pytest.fixture
def sample_health_check_results() -> dict[str, dict[str, bool | str]]:
