...
class TestVectorStoreServiceAssignments:
    ...
    def test_get_assigned_users_returns_validated_users(
        ...
        mock_result.stdout = json.dumps(
            {
                "items": [
                    {
                        "spec": {
                            "resourceRef": {"name": "test-vs"},
                            "subjects": {
                                "users": [{"name": "user1"}, {"name": "user2"}]
                            },
                        },
                        "status": {
                            "conditions": [{"type": "ValidatedUsers", "status": "True"}]
                        },
                    }
                ]
            }
        ...
        users: list[str] = service.get_assigned_users("test-vs", use_cache=False)
        ...
        mock_cache_service.delete.assert_called_once_with("domain_users_test-vs")
    ...
    def test_get_assigned_groups_returns_validated_groups(
                ...
                "items": [
                    {
                        ...
                        "status": {
                            "conditions": [
                                {"type": "ValidatedGroups", "status": "True"}
                            ]
                        },
                    }
                ]
        ...
        groups: list[str] = service.get_assigned_groups("test-vs", use_cache=False)
        ...
        mock_cache_service.delete.assert_called_once_with("domain_groups_test-vs")
        mock_cache_service.set.assert_called_once_with(
    ...
    def test_get_assigned_users_uses_users_cache_key(
        ...
        users: list[str] = service.get_assigned_users("test-vs")
    ...
    def test_get_assigned_groups_uses_groups_cache_key(
        ...
        groups: list[str] = service.get_assigned_groups("test-vs")
