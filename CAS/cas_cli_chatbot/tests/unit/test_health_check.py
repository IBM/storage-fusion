...
class TestHealthCheckerOverall:
    ...
    @pytest.mark.unit
    @pytest.mark.health
    def test_run_all_checks_executes_all_services(
        self: Any, mock_logger: Any
    ) -> None:
        ...
        services["vector store"].list_vector_stores.return_value = ["vs1", "vs2"]
        ...
        with patch("chatbot.utils.health_check.subprocess.run") as mock_run:
            ...
            results: dict[str, dict[str, Any]] = health_checker.run_all_checks()
    ...
    @pytest.mark.unit
    @pytest.mark.health
    def test_results_include_timestamp(self: Any, mock_logger: Any) -> None:
        """TC-HEALTH-002: Verify results include timestamp for each check"""
        services: dict[str, Mock] = {"auth": Mock()}
        services["auth"].token = "test-token"
    ...
    @pytest.mark.unit
    @pytest.mark.health
    def test_exception_handling_for_failed_checks(
        self: Any, mock_logger: Any
    ) -> None:
...
class TestHealthCheckerIndividualServices:
    ...
    @pytest.mark.unit
    @pytest.mark.health
    def test_auth_service_healthy_when_token_exists(
        self: Any, mock_logger: Any
    ) -> None:
        ...
        health_checker = HealthChecker(services, mock_logger)
        result: dict[str, Any] = health_checker.check_auth_service()
    ...
    @pytest.mark.unit
    @pytest.mark.health
    def test_auth_service_unhealthy_when_no_token(
        self: Any, mock_logger: Any
    ) -> None:
    ...
    @pytest.mark.unit
    @pytest.mark.health
    def test_cache_service_includes_hit_rate_statistics(
        self: Any, mock_logger: Any
    ) -> None:
        """TC-HEALTH-006: Verify cache service check includes hit rate statistics"""
        services: dict[str, Mock] = {"cache": Mock()}
        services["cache"].get_statistics.return_value = {
            "entries": 25,
        ...
        health_checker = HealthChecker(services, mock_logger)
        result: dict[str, Any] = health_checker.check_cache_service()
    ...
    @pytest.mark.unit
    @pytest.mark.health
    def test_user_service_attempts_to_fetch_users(
        self: Any, mock_logger: Any
    ) -> None:
        """TC-HEALTH-007: Verify user service check attempts to fetch users"""
        services: dict[str, Mock] = {"user": Mock()}
        services["user"].list_oc_users.return_value = ["user1", "user2", "user3"]

        health_checker = HealthChecker(services, mock_logger)
        result: dict[str, Any] = health_checker.check_user_service()
    ...
    @pytest.mark.unit
    @pytest.mark.health
    def test_vector_store_service_counts_available_stores(
        self: Any, mock_logger: Any
    ) -> None:
        """TC-HEALTH-008: Verify vector store service check counts available vector stores"""
        services: dict[str, Mock] = {"vector store": Mock()}
        services["vector store"].config = {"cas_namespace": "ibm-cas"}
        ...
        health_checker = HealthChecker(services, mock_logger)
        result: dict[str, Any] = health_checker.check_vector_store_service()
    ...
    @pytest.mark.unit
    @pytest.mark.health
    def test_query_service_validates_cas_url_configuration(
        self: Any, mock_logger: Any
    ) -> None:
        """TC-HEALTH-009: Verify query service check validates CAS URL configuration"""
        services: dict[str, Mock] = {"query": Mock()}
        services["query"].config = {"cas_url": "https://cas-api.test.com"}

        health_checker = HealthChecker(services, mock_logger)
        result: dict[str, Any] = health_checker.check_query_service()
    ...
    @pytest.mark.unit
    @pytest.mark.health
    def test_llm_service_lists_configured_providers(
        self: Any, mock_logger: Any
    ) -> None:
        """TC-HEALTH-010: Verify LLM service check lists configured providers"""
        services: dict[str, Mock] = {"llm": Mock()}
        services["llm"].config = {
            "llm_provider_sequence": ["nvidia", "openai", "ollama"]
        }

        health_checker = HealthChecker(services, mock_logger)
        result: dict[str, Any] = health_checker.check_llm_service()
    ...
    def test_oc_cli_validates_installation_and_version(
        self: Any, mock_run: Mock, mock_logger: Any
    ) -> None:
        ...
        result: dict[str, Any] = health_checker.check_oc_cli()
...
class TestHealthCheckerOCCLI:
    ...
    @patch("chatbot.utils.health_check.subprocess.run")
    def test_healthy_status_when_oc_installed(
        self: Any, mock_run: Mock, mock_logger: Any
    ) -> None:
    ...
    @pytest.mark.requires_oc
    @patch("chatbot.utils.health_check.subprocess.run")
    def test_unhealthy_status_when_oc_not_installed(
        self: Any, mock_run: Mock, mock_logger: Any
    ) -> None:
