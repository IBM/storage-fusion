...
class TestLLMServiceProviderConfiguration:
    ...
    @pytest.mark.unit
    @pytest.mark.llm
    def test_error_when_no_providers_configured(
        self: Any,
        sample_config: dict[str, Any],
        mock_logger: Any,
        mock_metrics_service: Any,
        ...
        llm_service = LLMService(sample_config, mock_logger, mock_metrics_service)

        result: str | None = llm_service.call_llm(sample_query_result, "test query")
    ...
    def test_providers_tried_in_sequence_order(
        self: Any,
        mock_openai: Mock,
        mock_nvidia: Mock,
        sample_config: dict[str, Any],
...
class TestLLMServiceOpenAIProvider:
    ...
    def test_successful_openai_call(
        self: Any,
        ...
        mock_stream: list[Mock] = [mock_chunk]
    ...
    @pytest.mark.unit
    @pytest.mark.llm
    def test_error_when_api_key_missing(
        self: Any,
        sample_config: dict[str, Any],
        mock_logger: Any,
        mock_metrics_service: Any,
    ...
    @pytest.mark.unit
    @pytest.mark.llm
    def test_error_when_api_key_is_placeholder(
        self: Any,
        sample_config: dict[str, Any],
        mock_logger: Any,
        mock_metrics_service: Any,
        ...
        llm_service = LLMService(sample_config, mock_logger, mock_metrics_service)
        result: str | None = llm_service._call_openai("test prompt")
...
class TestLLMServiceOllamaProvider:
    ...
    def test_successful_ollama_call(
        ...
        llm_service = LLMService(sample_config, mock_logger, mock_metrics_service)
        result: str | None = llm_service._call_ollama("test prompt")
    ...
    @pytest.mark.unit
    @pytest.mark.llm
    def test_default_host_used_when_not_configured(
        self: Any, mock_logger: Any, mock_metrics_service: Any
    ) -> None:
        ...
        assert (
            llm_service.config.get("ollama_host", "http://localhost:11434")
...
class TestLLMServiceNVIDIAProvider:
    ...
    @pytest.mark.llm
    @patch("chatbot.services.llm_service.requests.post")
    def test_successful_nvidia_call(
        self: Any,
        mock_post: Mock,
        sample_config: dict[str, Any],
        mock_logger: Any,
        ...
        # Use a longer response that will pass validation (needs at least 5 words and 10 chars)
        mock_response.json.return_value = {
            "choices": [
                {
                    "message": {
                        "content": "This is a valid NVIDIA response with sufficient content for validation."
                    }
                }
            ]
        }
        ...
        llm_service = LLMService(sample_config, mock_logger, mock_metrics_service)
        result: str | None = llm_service._call_nvidia("test prompt")

        assert (
            result
            == "This is a valid NVIDIA response with sufficient content for validation."
        )

    @pytest.mark.unit
    @pytest.mark.llm
    def test_error_when_nvidia_url_not_configured(
        self: Any, mock_logger: Any, mock_metrics_service: Any
    ) -> None:
        """TC-LLM-016: Verify error when NVIDIA URL is not configured"""
        config: dict[str, int] = {"llm_timeout": 60}
        llm_service = LLMService(config, mock_logger, mock_metrics_service)
    ...
    @pytest.mark.llm
    @patch("chatbot.services.llm_service.requests.post")
    def test_error_handling_for_api_failures(
        self: Any,
        mock_post: Mock,
        sample_config: dict[str, Any],
        mock_logger: Any,
...
class TestLLMServicePromptBuilding:
    ...
    @pytest.mark.unit
    @pytest.mark.llm
    def test_prompt_includes_search_result_data(
        self: Any,
        sample_config: dict[str, Any],
        mock_logger: Any,
        mock_metrics_service: Any,
        ...
        prompt: str = llm_service._build_prompt(sample_query_result, "test query")
    ...
    @pytest.mark.unit
    @pytest.mark.llm
    def test_prompt_includes_user_query(
        self: Any,
        sample_config: dict[str, Any],
        mock_logger: Any,
        mock_metrics_service: Any,
        ...
        prompt: str = llm_service._build_prompt({}, "What is the answer?")

        assert "What is the answer?" in prompt

    @pytest.mark.unit
    @pytest.mark.llm
    def test_search_result_serialization_for_dict(
        self: Any,
        sample_config: dict[str, Any],
        mock_logger: Any,
        mock_metrics_service: Any,
        ...
        result: dict[str, Any] = llm_service._serialize_search_result({"key": "value"})
    ...
    def test_search_result_serialization_with_to_dict_method(
        ...
        llm_service = LLMService(sample_config, mock_logger, mock_metrics_service)
        result: dict[str, Any] = llm_service._serialize_search_result(mock_obj)

        assert result == {"serialized": "data"}

    @pytest.mark.unit
    @pytest.mark.llm
    def test_search_result_serialization_when_none(
        self: Any,
        sample_config: dict[str, Any],
        mock_logger: Any,
        mock_metrics_service: Any,
        ...
        result: dict[str, Any] = llm_service._serialize_search_result(None)
...
class TestLLMServiceMetricsTracking:
    ...
    @pytest.mark.llm
    @patch("chatbot.services.llm_service.LLMService._call_nvidia")
    def test_timing_metrics_recorded(
        self: Any,
        mock_nvidia: Mock,
        sample_config: dict[str, Any],
        mock_logger: Any,
...
class TestLLMServiceProviderStatus:
    ...
    @pytest.mark.unit
    @pytest.mark.llm
    def test_get_provider_status_returns_all_configured(
        self: Any,
        sample_config: dict[str, Any],
        mock_logger: Any,
        mock_metrics_service: Any,
        ...
        llm_service = LLMService(sample_config, mock_logger, mock_metrics_service)
        status: dict[str, dict[str, Any]] = llm_service.get_provider_status()
    ...
    @pytest.mark.unit
    @pytest.mark.llm
    def test_provider_status_includes_model_and_url(
        self: Any,
        sample_config: dict[str, Any],
        mock_logger: Any,
        mock_metrics_service: Any,
