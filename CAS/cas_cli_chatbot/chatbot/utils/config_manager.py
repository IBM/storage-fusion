...
class ConfigManager:
    ...
    def prompt_for_credentials(self) -> dict[str, str]:
        ...
        try:
            ...
            use_generated = Confirm.ask("[yellow]Use this CAS URL?[/]", default=True)

            if not use_generated:
                cas_url = Prompt.ask("[yellow]Enter CAS URL manually[/]")
                # Strip trailing slash if present
                cas_url = cas_url.rstrip("/")
                namespace = Prompt.ask("[yellow]Enter CAS Namespace[/]")
    ...
    def create_config_from_sample(self, credentials: dict[str, str]) -> dict[str, Any]:
        """Create config from sample file with provided credentials"""
        if not self.sample_path.exists():
            raise FileNotFoundError(f"Sample config not found: {self.sample_path}")
    ...
    def save_config(self, config: dict[str, Any], backup: bool = True) -> None:
        ...
        console.print(f"[bold green]✓ Configuration saved to {self.config_path}[/]")
    ...
    def update_default_vector_store(self, vector_store_name: str) -> None:
        """
        ...
        if not self.config_path.exists():
            console.print(
                "[yellow]⚠ Config file not found. Cannot update default vector store.[/]"
            )
            return
    ...
    def prompt_for_llm_setup(self) -> dict[str, Any]:
        ...
        choice = Prompt.ask(
            "[yellow]Select provider[/]", choices=["1", "2", "3", "4"], default="4"
        )
        ...
        if choice == "1":
            ...
            model = Prompt.ask("[yellow]Model[/]", default="gpt-3.5-turbo")
            ...
            console.print("[green][bold]✓ OpenAI configured[/]")

        elif choice == "2":
            # Ollama setup
            console.print("\n[bold cyan]Ollama Configuration[/]")
            host = Prompt.ask(
                "[yellow]Ollama Host URL[/]", default="http://localhost:11434"
            )
            model = Prompt.ask("[yellow]Model[/]", default="llama3")
            ...
            console.print("[green][bold]✓ Ollama configured[/]")

        elif choice == "3":
            # NVIDIA setup
            console.print("\n[bold cyan]NVIDIA NIM Configuration[/]")
            url = Prompt.ask("[yellow]NVIDIA Endpoint URL[/]")
            model = Prompt.ask("[yellow]Model[/]", default="meta/llama-3.2-1b-instruct")
            ...
            console.print("[green][bold]✓ NVIDIA configured[/]")

        else:
            console.print(
                "[dim]Skipping LLM setup. You can configure it later using the config file.[/]"
            )
    ...
    def update_llm_config(self, llm_config: dict[str, Any]) -> None:
        ...
        if not self.config_path.exists():
            console.print(
                "[yellow]⚠ Config file not found. Cannot update LLM configuration.[/]"
            )
            return
    ...
    def _extract_cluster_from_console_url(self, console_url: str) -> str:
        ...
        try:
            ...
            raise ValueError(f"Failed to extract cluster from console URL: {e}") from e
