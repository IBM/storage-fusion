...
from chatbot.utils.validators import InputValidator, TokenValidator, ValidationError


class CacheServiceProtocol(Protocol):
    """Protocol for cache operations used by query service."""

    def get(self, key: str) -> Any: ...

    def set(self, key: str, value: Any, ttl_seconds: int) -> None: ...

    def clear_pattern(self, pattern: str) -> Any: ...
...
class QueryService:
    ...
    def query_vector_store(
        ...
        if not token_check["valid"]:
            self.logger.error(f"Token validation failed: {token_check['error']}")
        ...
        if use_cache and self.cache_service:
            ...
            if cached is not None:
                self.logger.debug(f"Retrieved query results from cache: {vector_store}")
        ...
        try:
            ...
            payload = {
                ...
                "enable_content_metadata": self.config.get(
            ...
            try:
                ...
                # Extract the "detail" key from the error response
                if "detail" in error_body:
                    error_message = error_body["detail"]
                elif "message" in error_body:
                    error_message = error_body["message"]
                elif "error" in error_body:
                    error_message = error_body["error"]
            except (ValueError, AttributeError):
                # If response is not JSON, capture raw text
    ...
    def list_vector_stores(self, use_cache: bool = True) -> list[str]:
        """
        ...
        try:
            ...
            if self.cache_service:
                self.cache_service.set(
                    cache_key, vector_stores, ttl_seconds=600
                )  # 10 minutes
    ...
    def get_vector_store_info(self, vector_store: str) -> dict[str, Any] | None:
        """
        ...
        try:
            ...
            data_list = cast(list[dict[str, Any]], response_data.get("data", []))

            # Get the info for the specified vector store
            vector_store_info = next(
                (item for item in data_list if item.get("name") == vector_store), None
            )
    ...
    def get_file_content(self, vector_store_id: str, file_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            return ResponseFormatter.timeout_error(
                "File content retrieval", self.timeout
            )
        except requests.exceptions.RequestException as e:
