...
class ChatbotCLI:
    ...
    COMMANDS = {
        ...
        "vector stores info files": "Show file counts, bytes, and storage details",
        "llm query file": "Query a specific file from a vector store",
        "llm setup": "Configure an LLM provider",
        "help": "Show available commands",
        "vector stores list": "List all available vector stores",
        "vector stores select": "Select a vector store to work with",
        "vector stores info users": "Show user assignments and access",
        "llm query ask": "Ask a query using LLM",
    ...
    def __init__(
        ...
        self.current_user: str | None = config.get("oc_username")
        self.user_type = "ocp"
        self.current_namespace: str | None = config.get("cas_namespace")
        self.current_vector_store: str | None = None
        self.running = True
    ...
    def _get_query_input(self) -> str | None:
    ...
    def _execute_query_with_llm(self, query: str) -> bool:
        ...
        # Validate search_result is a valid dict
        if not isinstance(search_result, dict):
            self.console.print("[red]✗ Invalid search result format[/]")
            self.logger.error("Search result is not a dictionary")
            return False

        # Extract data from search result (CAS API returns {"data": [...], ...})
        data = search_result.get("data")
        
        # Validate data exists and is a non-empty list
        if not data:
            self.console.print(
        ...
        if not isinstance(data, list):
            self.console.print("[red]✗ Invalid data format in search result[/]")
            self.logger.error(f"Search data is not a list: {type(data)}")
            return False

        self.console.print("\n[bold cyan]Getting AI response...[/]\n")
        llm_result = self.services["llm"].call_llm(data, query)

        self._record_successful_query(query, llm_result)
        return True

    def _get_active_vector_store(self) -> str | None:
        """Get the active vector store, prioritizing current selection over default."""
        vector_store = self.current_vector_store or str(
            self.config.get("default_vector_store", "")
        )
        if not vector_store:
            self.console.print("[red]✗ No vector store selected or configured[/]")
            return None
        return vector_store

    def _record_successful_query(
        self, query: str, search_result: str | list[str] | None
    ) -> None:
    ...
    def _get_file_identifiers(self) -> tuple[str, str]:
        ...
        vector_store_id = self._get_input(
            ...
            "Enter the vector store ID (same as vector store name): "
        )
    ...
    def _retrieve_file_content(
        ...
        file_content = cast(
            dict[str, Any],
            self.services["query"].get_file_content(
                vector_store_id=vector_store_id,
                file_id=file_id,
            ),
        )

        if not self._retrieved_result_valid(file_content, "File content retrieval"):
            return None
    ...
    def _execute_vector_search(
        ...
        search_result = cast(
            dict[str, Any],
            self.services["query"].query_vector_store(
                user_query=query,
    ...
    def _retrieved_result_valid(self, result: Any, content_type: str) -> bool:
        """Validate retrieval result."""
        # Check 0: Result must not be None
        if result is None:
            self.console.print(f"[yellow]{content_type}: No matching results found...")
            return False
        
        # Check 1: Result must be a dictionary
        if not isinstance(result, dict):
            self.console.print(f"[red]✗ {content_type} failed: Invalid response format[/]")
            self.logger.error(f"{content_type} returned non-dict result for {self.current_user}")
            return False
        
        # Check 2: Explicit error checking - default to False for safety
        # If "success" key exists and is False, it's an error
        if "success" in result and not result.get("success"):
            ...
            if "bearer token" in error.lower() or "token" in error.lower():
                self.console.print("[bold yellow]⚠ No valid bearer token available.[/]")
                self.console.print(
    ...
    def _get_input(self, prompt: str) -> str:
        ...
        while True:
            user_input = str(self.session.prompt(prompt)).strip()
            if not user_input:
    ...
    def _has_valid_llm_config(self) -> bool:
        ...
        if not self.config_manager:
            return False
        return bool(self.config_manager.has_llm_configured(self.config))
    ...
    def cmd_vector_stores_info_users_files(self, *args: Any, **kwargs: Any) -> Any:
        return handlers.cmd_vector_stores_info_users_files(self, *args, **kwargs)

    def prompt_for_vector_store_selection(self, *args: Any, **kwargs: Any) -> Any:
