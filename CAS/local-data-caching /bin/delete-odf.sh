...
for SC in $OCS_STORAGECLASSES; do
	...
	echo "======================================================================"
	result=$(oc get pvc --all-namespaces --no-headers 2>/dev/null | grep "$SC" | grep -c -v -e "$NOOBAA_DB_PVC" -e "$NOOBAA_BACKINGSTORE_PVC")
	if [[ $result -eq 0 ]]; then
		info "no pvc using $SC"
	else
		has_pvc_obc=1
		error "has pvc using $SC, please remove them first"
		oc get pvc --all-namespaces --no-headers 2>/dev/null | grep "$SC" | grep -v -e "$NOOBAA_DB_PVC" -e "$NOOBAA_BACKINGSTORE_PVC"
	fi

	result=$(oc get obc --all-namespaces --no-headers 2>/dev/null | grep -c "$SC")
	if [[ $result -eq 0 ]]; then
		info "no obc using $SC"
	else
		has_pvc_obc=1
		error "has obc using $SC, please remove them first"
		oc get obc --all-namespaces --no-headers 2>/dev/null | grep "$SC"
	fi
	echo
done
if [[ $has_pvc_obc -eq 1 ]]; then
	error "please check from above output, there is pvc/obc using odf, need clean them up first."
	exit 1
else
	info "pvc obc check done"
fi
